...
int main()
{
    ...
    auto gpuInCfg = yst::core::BufferBuilder(yst::core::BufferPreset::Empty)
                        .WithSize(dataSize)
                        .WithUsage(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT) // Accepts copying
                        .Build();
    auto [gpuIn, gInErr] = yst::core::CreateBuffer(device, gpuInCfg);

    auto gpuOutCfg = yst::core::BufferBuilder(yst::core::BufferPreset::Empty)
                         .WithSize(dataSize)
                         .WithUsage(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT) // Transfers back
                         .Build();
    ...
    auto bglCfg = yst::core::BindGroupLayoutBuilder(
        yst::core::BindGroupLayoutPreset::Empty)
                      .AddStorageBuffer(
                          0, true,
                          yst::core::ShaderStageBits::Compute) // ReadOnly = true
                      .AddStorageBuffer(
                          1, false,
                          yst::core::ShaderStageBits::Compute) // ReadOnly = false
                      .Build();
    ...
    auto submitErr = yst::core::SubmitOneTimeCommands(
        device, [&](yst::core::CommandList& cmd) -> yst::CustomError {
            cmd.Begin();
            ...
            cmd.PipelineBarrierBuffer(
                gpuIn.buffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT);

            // 5.3. Computing
            cmd.BindPipeline(computePipeline.pipeline,
                yst::core::PipelineBindPoint::Compute);
            cmd.BindBindGroup(pipelineLayout.layout, 0, bg,
                yst::core::PipelineBindPoint::Compute);
            cmd.Dispatch(16, 1, 1); // 1024 / 64 = 16 groups

            // 5.4. Barrier SHADER_WRITE -> TRANSFER_READ
            cmd.PipelineBarrierBuffer(
                gpuOut.buffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
